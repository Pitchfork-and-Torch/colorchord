...
export function LumenApp() {
  ...
  const shareApp = async () => {
    ...
    const data = {
      title: "ColorChord 2.2.0 - Living Spectrum",
      ...
      {!uiHidden && (
        <header className="pointer-events-none absolute inset-x-0 top-0 z-20 flex items-start justify-between gap-1.5 p-2 pt-[max(0.4rem,env(safe-area-inset-top))] sm:gap-2 sm:p-5">
          <div className="pointer-events-auto max-w-[min(100%,17.5rem)] rounded-[var(--radius-md)] bg-[var(--color-bg)]/80 px-2 py-1.5 shadow-lg backdrop-blur-md sm:max-w-[min(100%,20rem)] sm:bg-transparent sm:px-0 sm:py-0 sm:shadow-none sm:backdrop-blur-none">
            ...
            {chordError && (
              <p className="mt-1 max-w-[17rem] text-[0.62rem] leading-snug text-red-300/90" role="alert">{chordError}</p>
            )}
          </div>
        ...
        >
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <button
              ...
              aria-pressed={multiSelect.length > 0}
              className={cn(
            ...
            <div className="flex flex-wrap gap-1.5">
              {PROGRESSIONS.map((p) => {
                ...
                return (
                  ...
                  >
                    {active ? <Pause className="size-3" aria-hidden /> : <Play className="size-3" aria-hidden />}
                    {p.name}
                  </button>
                );
              })}
          ...
          <div className="flex flex-wrap items-center gap-1 border-t border-[var(--color-border)] pt-2">
            <Button variant="ghost" size="sm" onClick={() => setExpandedSpectrum((v) => !v)} className="gap-1 min-h-10" aria-pressed={expandedSpectrum}>
              {expandedSpectrum ? <Eye className="size-3.5" /> : <EyeOff className="size-3.5" />}
              UV/IR
            </Button>
            <Button variant="ghost" size="sm" onClick={() => setShowGeometry((v) => !v)} className="gap-1 min-h-10" aria-pressed={showGeometry}>
            ...
            <Button variant="ghost" size="sm" onClick={() => setTheoryOpen((v) => !v)} className="gap-1 min-h-10" aria-pressed={theoryOpen}>
              Theory
            </Button>
            <Button
              variant="ghost"
