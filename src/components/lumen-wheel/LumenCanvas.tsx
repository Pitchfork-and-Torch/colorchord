...
export function LumenCanvas({
  ...
  const hitTest = useCallback((clientX: number, clientY: number): PitchClass | null => {
    ...
    const w = rect.width;
    const h = rect.height;
    const { rotation: rot, tilt: tiltNow, pureLight } = stateRef.current;
    ...
    if (dist < size * 0.096 || dist > outerR * 1.14) return null;
  ...
  useEffect(() => {
    ...
    const pivot = (clientX: number, clientY: number) => {
      const rect = canvas.getBoundingClientRect();
      const { tilt: tiltNow } = stateRef.current;
      const parallaxX = (tiltNow.gamma / 45) * Math.min(14, rect.width * 0.02);
      const parallaxY = (tiltNow.beta / 45) * Math.min(14, rect.height * 0.02);
      return {
        cx: rect.width / 2 + parallaxX,
        cy: rect.height / 2 + parallaxY,
        x: clientX - rect.left,
        y: clientY - rect.top,
      };
    };
    ...
    const onDown = (e: PointerEvent) => {
      const p = pivot(e.clientX, e.clientY);
      dragRef.current = {
        dragging: true,
        lastAngle: angleAt(p.cx, p.cy, p.x, p.y),
        moved: false,
    ...
    const onMove = (e: PointerEvent) => {
      ...
      const p = pivot(e.clientX, e.clientY);
      const ang = angleAt(p.cx, p.cy, p.x, p.y);
