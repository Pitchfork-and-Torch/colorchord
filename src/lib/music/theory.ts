...
export function chordMidis(rootMidi: number, quality: ChordQuality): number[] {
  ...
  switch (quality) {
    ...
    case "min9":
      return [r, (r + 3) % 12, (r + 7) % 12, (r + 10) % 12, (r + 2) % 12];
    case "min7":
...
export function theoryBlurb(root: PitchClass, quality: ChordQuality, tones: PitchClass[]): string {
  ...
  if (quality === "min9") {
    return `Min9 ${labels}: minor seventh plus the ninth - cooler color with an open ninth lift. Hue span ~${Math.round(span)}°; center light ≈ ${mix}. ${names}.`;
  }
  if (quality === "min7") {
...
function parseQualityToken(token: string): ChordQuality | null {
  ...
  if (t === "m9" || t === "min9" || t === "minor9" || t === "-9" || t === "mi9") return "min9";
...
export function parseChordSymbol(raw: string): ParsedChord | null {
  ...
  if (words) {
    ...
    else if (/^(minor\s*9|min\s*9)$/.test(rest)) qualToken = "min9";
