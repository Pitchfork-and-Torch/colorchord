...
export function chordMidis(rootMidi: number, quality: ChordQuality): number[] {
  ...
  switch (quality) {
    ...
    case "dim7":
      // Fully diminished seventh: stacked minor thirds (m3 + d5 + d7)
...
export function theoryBlurb(root: PitchClass, quality: ChordQuality, tones: PitchClass[]): string {
  ...
  if (quality === "dim7") {
    return `Dim 7 ${labels}: four stacked minor thirds (fully diminished). Symmetric tension around the ring — hue span ~${Math.round(span)}°, center reads as ${mix}. ${names}.`;
  }
  if (quality === "aug") {
